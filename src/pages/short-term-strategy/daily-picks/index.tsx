import type { DailyPicksData, DailyPickStock, StockDeepAnalysis } from "#src/api/strategy";
import { fetchDailyPicks, refreshDailyPicks } from "#src/api/strategy";
import {
	AlertOutlined,
	BulbOutlined,
	FireFilled,
	...
	return (
		<div style={{ marginTop: 12 }}>
			...
			>
				...
				<div>
					<Text style={{ fontSize: 12, lineHeight: "20px" }}>{deep.moat}</Text>
				</div>
				{deep.main_business && (
					<div style={{ marginTop: 4 }}>
						<Text style={{ fontSize: 12, color: "#595959" }}>
			...
			>
				...
				<Row gutter={12}>
					<Col span={12}>
						...
						<div>
							<Text style={{ fontSize: 12 }}>{deep.revenue_trend}</Text>
						</div>
					</Col>
					<Col span={12}>
						<Text style={{ fontSize: 11, color: "#8c8c8c" }}>净利润（近3季）</Text>
						<div>
							<Text style={{ fontSize: 12 }}>{deep.profit_trend}</Text>
						</div>
					</Col>
				</Row>
			...
			>
				...
				<div>
					<Text style={{ fontSize: 12, lineHeight: "20px" }}>{deep.recent_move_reason}</Text>
				</div>
			</div>
			...
			{deep.deep_summary && (
				...
				>
					...
					<div>
						<Text style={{ fontSize: 12, lineHeight: "20px", color: "#391085" }}>{deep.deep_summary}</Text>
					</div>
				</div>
			)}
		</div>
		...
		<Card bordered={false} style={cardStyle} bodyStyle={{ padding: 16 }}>
			...
			<div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
				<Space size={8} align="center">
					...
					<div>
						...
						{cfg && (
							<div>
								<Text style={{ fontSize: 11, color: "#8c8c8c" }}>{cfg.label}</Text>
							</div>
						)}
					</div>
				</Space>
			...
			<div style={{ marginBottom: 8 }}>
				<Space wrap size={[4, 4]}>
					...
					{stock.is_combined && (
						<Tag color="purple" style={{ margin: 0, fontSize: 11 }}>🔗多战法共振</Tag>
					)}
				</Space>
			</div>
...
/** 本周推荐核心逻辑 Card */
const WeeklyLogicCard: React.FC<{ data: DailyPicksData }> = ({ data }) => {
	...
	return (
		...
		>
			<Space direction="vertical" style={{ width: "100%" }} size={12}>
				...
				>
					<Paragraph style={{ color: "rgba(255,255,255,0.85)", margin: 0, fontSize: 13, lineHeight: "22px" }}>
						{logic.weekly_logic}
					</Paragraph>
				</div>
				...
				<Row gutter={16}>
					...
					<Col span={10}>
						...
						<div style={{ marginTop: 4 }}>
							<Text style={{ color: "rgba(255,255,255,0.72)", fontSize: 12 }}>{logic.operation_advice}</Text>
						</div>
					</Col>
				</Row>
		...
		return (
			<div style={{ padding: 24 }}>
				...
				<div style={{ marginTop: 16 }}>
					{[1, 2, 3].map(i => (
						<Skeleton key={i} active paragraph={{ rows: 6 }} style={{ marginBottom: 16 }} />
					))}
				</div>
			</div>
		);
			...
			<div style={{ padding: 24 }}>
				...
				<Card>
					<Empty description="当前暂无强烈推荐股票" image={Empty.PRESENTED_IMAGE_SIMPLE}>
						<Text type="secondary">
							各战法还未生成推荐，或当前没有达到「强烈推荐」级别的股票，请等待各战法更新后再查看
						</Text>
						<div style={{ marginTop: 16 }}>
		...
		<div style={{ padding: 24 }}>
				...
				<Row gutter={[24, 16]} align="middle">
					...
					<Col xs={24} sm={10}>
						<Row gutter={16} justify="end">
							<Col>
								<Statistic
									title={<span style={{ color: "rgba(255,255,255,0.65)" }}>精选股数</span>}
									value={data.total}
							...
							<Col>
								<Statistic
									title={<span style={{ color: "rgba(255,255,255,0.65)" }}>覆盖战法</span>}
			...
			{restPicks.length > 0 && (
				<>
					...
					{restPicks.map(stock => (
						<PickCard key={stock.code} stock={stock} />
					))}
				</>
			)}
		</div>
