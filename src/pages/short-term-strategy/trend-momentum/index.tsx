...
const TrendMomentumPage: React.FC = () => {
	const [data, setData] = useState<TrendMomentumData | null>(null);
	...
	const fetchData = async () => {
		...
		setError(null);
		try {
			const response = await fetchTrendMomentumRecommendations(13);
	...
	const handleRefresh = async () => {
		...
		try {
			const response = await refreshTrendMomentumRecommendations(13);
			if (response.status === "success") {
				setData(response.data);
	...
	const columns: ColumnsType<TrendMomentumStock> = [
		{
			title: "排名",
			dataIndex: "rank",
			key: "rank",
		...
		},
		{
			...
			key: "stock",
			width: 140,
			render: (_: any, record: TrendMomentumStock) => (
		...
		},
		{
			...
			width: 120,
			render: (type: string) => (
				<Tag color={signalColors[type] || "default"} icon={<RiseOutlined />}>
					{type}
				</Tag>
			),
		},
		...
		},
		{
			...
			render: (v: number | null) => (
				v != null
					? (
						<Text style={{ color: v >= 25 ? "#52c41a" : "#8c8c8c", fontWeight: "bold" }}>
							{v.toFixed(0)}
						</Text>
					)
			...
			<div style={{ padding: 24 }}>
				<Card>
					<Empty description="暂无趋势动量信号" image={Empty.PRESENTED_IMAGE_SIMPLE}>
						<Text type="secondary">当前暂无符合条件的趋势动量信号股，请在交易时间段内查看</Text>
					</Empty>
				</Card>
			</div>
		);
...
export default TrendMomentumPage;
