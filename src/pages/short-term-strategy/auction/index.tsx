...
const AuctionPage: React.FC = () => {
	...
	const fetchData = async () => {
		...
		setError(null);
		try {
			const response = await fetchAuctionRecommendations(13);
	...
	const handleRefresh = async () => {
		...
		try {
			const response = await refreshAuctionRecommendations(13);
			if (response.status === "success") {
				setData(response.data);
	...
	const columns: ColumnsType<AuctionStock> = [
		{
			title: "排名",
			dataIndex: "rank",
			key: "rank",
		...
		},
		{
			...
			key: "stock",
			width: 140,
			render: (_: any, record: AuctionStock) => (
		...
		},
		{
			...
			render: (type: string) => (
				<Tag
					color={type.includes("竞价") ? "gold" : "purple"}
					icon={type.includes("竞价") ? <SunOutlined /> : <ClockCircleOutlined />}
				>
					{type}
				</Tag>
			),
		},
		{
			...
			width: 90,
			align: "right",
			render: (v: number, record: AuctionStock) => {
				if (record.signal_type.includes("尾盘"))
		...
		},
		{
			...
			render: (v: number, record: AuctionStock) => {
				if (record.signal_type.includes("尾盘")) {
					const nearHigh = record.near_high_pct ?? 100;
					return (
						<Text style={{ color: nearHigh <= 5 ? "#52c41a" : "#fa8c16" }}>
							距高点
							{" "}
							{nearHigh.toFixed(1)}
							%
				...
				return (
					<Text style={{ color: v > 0 ? "#52c41a" : "#f5222d", fontWeight: "bold" }}>
						{v > 0 ? "走强" : "走弱"}
						{" "}
		...
		},
		{
			...
			render: (v: number) => (
				<Text
					strong
					style={{
						color: v >= 80 ? "#f5222d" : v >= 60 ? "#fa8c16" : "#1890ff",
						fontSize: 16,
					}}
				>
					{v}
	...
	if (!data || data.recommendations.length === 0) {
		return (
			<div style={{ padding: 24 }}>
				<Card>
					<Empty description="暂无竞价/尾盘信号" image={Empty.PRESENTED_IMAGE_SIMPLE}>
						<Text type="secondary">竞价信号在9:15-9:30生成，尾盘信号在14:30-15:00生成</Text>
					</Empty>
				</Card>
			</div>
		);
	...
	const gradientMap: Record<string, string> = {
		集合竞价: "linear-gradient(135deg, #f6d365 0%, #fda085 100%)",
		尾盘战法: "linear-gradient(135deg, #a18cd1 0%, #fbc2eb 100%)",
		综合分析: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",
	};
