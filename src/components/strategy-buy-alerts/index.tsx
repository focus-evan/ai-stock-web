import type { StrategyBuyAlert, StrategyFollowType } from "#src/api/strategy";
...
function BuyCard({ item, now, unavailable }: { item: StrategyBuyAlert, now: number, unavailable: boolean }) {
	...
	const expires = Date.parse(follow.expires_at);
...
export default function StrategyBuyAlerts({ userId }: { userId: string }) {
	...
	const day = beijingTradeDate(new Date(now));
	const query = useQuery({
		queryKey: ["today-buy-alerts", userId, day],
		queryFn: async ({ signal }) => {
			const response = await fetchTodayStrategyBuys(signal);
			if (response.status !== "success" || !response.data)
				throw new Error(response.message || "今日买入读取失败");
	...
	const title = items.length ? `今日模拟买入 · ${items.length} 笔 · ${strategyCount} 个战法` : failed ? "今日模拟买入检查未完成" : query.isPending ? "正在检查全部战法今日买入…" : "全部14个战法 · 今日暂无模拟买入";
	return (
		<section aria-label="全部战法今日模拟买入" style={{ margin: "16px 16px 0", padding: 16, borderRadius: 12, border: `2px solid ${items.length ? "#ff4d4f" : "#d9d9d9"}`, background: items.length ? "#fff1f0" : "#fff" }}>
			<Space wrap style={{ width: "100%", justifyContent: "space-between" }}>
				<Text strong style={{ color: items.length ? "#cf1322" : undefined, fontSize: items.length ? 24 : 14 }}>{title}</Text>
				<Space wrap>
					<Text type="secondary">{`${day}（北京时间）· 每15秒更新`}</Text>
