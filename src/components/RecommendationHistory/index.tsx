...
const RecommendationHistory: React.FC<Props> = ({
	...
	const [lastDatesLen, setLastDatesLen] = useState(0);

	// 当日期列表变化时重置到最新日期
	if (sortedDates.length !== lastDatesLen) {
		setLastDatesLen(sortedDates.length);
		setDateIdx(0);
	}
	...
	return (
		<>
												...
												<div style={{ padding: "0 10px 10px" }}>
													<Button
														...
														onClick={() => setFollowUpStock({
															code,
															name,
															buyPrice: stock.suggested_buy_price || stock.buy_price_range,
															targetPrice: stock.suggested_sell_price || stock.target_price,
															stopLoss: stock.stop_loss_price,
															advice,
														})}
													>
														⚡ 跟投分析
			...
			<FollowUpModal
				open={!!followUpStock}
				onClose={() => setFollowUpStock(null)}
				stockCode={followUpStock?.code ?? ""}
				stockName={followUpStock?.name ?? ""}
				originalBuyPrice={followUpStock?.buyPrice}
				originalTargetPrice={followUpStock?.targetPrice}
				originalStopLoss={followUpStock?.stopLoss}
				originalAdvice={followUpStock?.advice}
			/>
		</>
	);
};
